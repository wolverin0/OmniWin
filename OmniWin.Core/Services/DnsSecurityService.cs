    ...
    public async Task<(bool Success, int BlockedDomainsCount, string Message)> SyncCommunityHostsBlocklistAsync()
    {
        ...
        try
        {
            ...
            var validLines = rawHosts.Split('\n')
                ...
                .Take(25000) // Safe top block limit to keep DNS cache performant
                .ToList();
            ...
            return (true, validLines.Count, $"Se aplicaron {validLines.Count:N0} dominios bloqueados en HOSTS exitosamente.");
        }
        catch (Exception ex)
        {
    ...
    public bool RestoreDefaultDns()
    {
        try
        {
            ...
            foreach (var iface in activeIfaces)
            {
                ...
                p?.WaitForExit(3000);
                if (p?.ExitCode == 0) anySuccess = true;
            }
            DnsFlush();
            return anySuccess;
        }
